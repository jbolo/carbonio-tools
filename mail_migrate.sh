...
function days_since_zimbra_timestamp
{
   ...
   if [[ "$COMPACT_TS" =~ ^([0-9]{4})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})([0-9]{2})$ ]]; then
      TS_EPOCH=`date -u -d "${BASH_REMATCH[1]}-${BASH_REMATCH[2]}-${BASH_REMATCH[3]} ${BASH_REMATCH[4]}:${BASH_REMATCH[5]}:${BASH_REMATCH[6]} UTC" "+%s" 2>/dev/null`
      NOW_EPOCH=`date -u "+%s"`
...
function count_mailbox_user
{
   ...
   while read -r j; do
      ...
      account_data=`${ZMPROV} -l ga "$j" zimbraAccountStatus zimbraLastLogonTimestamp zimbraCreateTimestamp zimbraPasswordModifiedTime zimbraMailHost zimbraMailQuota zimbraIsAdminAccount 2>/dev/null || true`
